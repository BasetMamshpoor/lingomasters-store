...
const Sellers = ({ sellers = [] }) => {
    return (
        <>
            <div className="sm:p-6 px-3 py-4 flex flex-col gap-6 bg-white rounded-lg border-natural_gray-100 border scroll-m-52" id='seller'>
                ...
                <ul className="flex flex-col gap-8">
                    {sellers.map((s, i) => {
                        return <li className='flex items-center justify-between gap-1' key={s.id}>
                            <span className='sm:text-base text-[10px] text-primary-950 line-clamp-1'>{s.name}</span>
