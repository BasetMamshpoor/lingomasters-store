                    ...
                    <>
                        <div className='flex flex-col items-stretch overflow-hidden relative' key={p.idp} id='product'>
                            <div className='flex gap-6 grow'>
                                <Link href={`/products/${p.id}`} className='h-[90px] w-[90px] flex-shrink-0'>
                                    <Image placeholder='blur' blurDataURL='/images/product.png' width={100} height={100} unoptimized={true} src='/images/product.png' alt="" />
                                </Link>
                                <div className='grow flex flex-col gap-2'>
                                    <h3 className='font-semibold sm:text-base text-sm'>کتاب طلسم مکالمه</h3>
                                    <div className='flex items-start mb-2 flex-col gap-2'>
                                        <div className='centerOfParent gap-2'>
                                            ...
                                            <p className="sm:text-sm text-xs">کتاب چاپی</p>
                                        </div>
                                        <div className='centerOfParent gap-2'>
                                            <div className='centerOfParent'>
                                                <Store className='sm:w-6 sm:h-6 w-4 h-4' />
                                            </div>
                                            <p className="sm:text-sm text-xs">سعید اسعدی</p>
                                        </div>
                                    </div>
                                    <div className={`md:flex items-center justify-between hidden w-full`}>
                                        <div className='flex items-center gap-1 flex-wrap md:flex-row flex-col-reverse'>
                                            <p className='text-xl text-green-500 flex items-center hasToman'>{formatCurrency(250000)}</p>
                                            {<del className='text-natural_gray-400 text-sm  flex items-center hasToman'>{formatCurrency(300000)}</del>}
                                        </div>
                                        <div className='flex items-center gap-2 p-2 border border-natural_gray-300 rounded justify-between w-fit'>
                                            <button type='button' className="centerOfParent" onClick={() => dispatch({ type: 'INCREASE', payload: paint })}>
                                                <Plus className="fill-primary-700 cursor-pointer w-4 h-4" />
                                            </button>
                                            <span className='sm:text-base centerOfParent text-natural_gray-800'>{1}</span>
                                            <button type='button' className='centerOfParent'>
                                                {i < 2 ?
                                                    <Trush className="fill-red-700 cursor-pointer w-5 h-5" onClick={() => dispatch({ type: "REMOVE_ITEM", payload: paint })} /> :
                                                    <Minus className="fill-primary-700 cursor-pointer w-4 h-4" onClick={() => dispatch({ type: "DECREASE", payload: paint })} />
                                                }
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div className={`flex items-center justify-between md:hidden`}>
                                <div className='flex items-center flex-wrap md:flex-row flex-col-reverse'>
                                    <p className='text-sm text-green-500 flex items-center hasToman'>{formatCurrency(250000)}</p>
                                    {<del className='text-natural_gray-400 text-xs hasToman'>{formatCurrency(300000)}</del>}
                                </div>
                                <div className='flex md:hidden items-center gap-2 border border-natural_gray-300 justify-between p-2 rounded'>
                                    ...
                                    <span className='text-sm centerOfParent text-natural_gray-800'>{i}</span>
