...
const Card = ({ withLabel = true, solid = false, offRed = false }) => {
    return (
        <>
            <div dir='ltr' className={`relative select-none overflow-hidden flex flex-col items-stretch sm:gap-3 gap-1 sm:w-[302px] w-[242px] h-[405px] sm:h-[528px] flex-shrink-0 rounded-lg border border-natural_gray-${solid ? "400" : '100'} md:p-6 p-4 bg-white`}>
                <div className="centerOfParent sm:w-[254px] w-[210px] sm:h-[250px] h-[200px] flex-shrink-0 rounded-lg mix-blend-darken">
                    <Image
                ...
                <div className="grow flex flex-col justify-between">
                    ...
                    <div className="flex items-center sm:gap-6 gap-4 sm:w-64 w-52">
                        <button className="centerOfParent bg-primary-500 p-4 sm:w-[60px] w-11 sm:h-12 h-8 rounded-md"><Cart /></button>
                        <Link href='' className='sm:h-12 h-8 flex-[1_0_0] sm:px-6 px-4 sm:py-4 py-2 rounded border-secondary-500 border-[1.5px] text-secondary-500 centerOfParent'>مشاهده</Link>
                    </div>
                </div>
