...
const Cart = () => {
    const {state, dispatch, isLoading} = useContext(CartContext)
    return (
        <>
            <main dir="rtl">
                <div className="container flex flex-col gap-20">
                    ...
                    <div className="grid lg:grid-cols-8 grid-cols-1 lg:gap-6 gap-y-4">
                        <div
                            className="col-span-6 p-6 border border-natural_gray-100 rounded-lg bg-white flex flex-col gap-10">
                            <div className="flex items-center justify-between">
                                <p className="font-bold">سبد خرید شما</p>
                                {!!state.itemsCounter && <div className="centerOfParent cursor-pointer"
                                                              onClick={() => dispatch({type: "CLEAR"})}><Brush/></div>}
                            </div>
                            ...
                            <div
                                ...
                                <div className="flex flex-col gap-3">
                                    <div
                                        ...
                                        <span className="hasToman text-sm">{isLoading ?
                                    ...
                                    {state.total !== state.total_after_off && <div
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            <span
                                                className="text-red-500 text-xs">{isLoading ?
                                                <Spinner variant="dots" size="sm"
                                    ...
                                    <div
                                        ...
                                        <span
                                            className="hasToman text-sm text-green-600">{isLoading ?
                                            <Spinner variant="dots" size="sm"
                                                     color="success"/> : formatCurrency(state.total_after_off)}</span>
                                    </div>
                                </div>
                                <Link href='/checkout/shipping'
                                      className="effect-2 text-white bg-primary-600 rounded h-12 lg:flex hidden items-center justify-center">ادامه
                                    ثبت سفارش</Link>
                                <div
                                    ...
                                    <Link href='/checkout/shipping'
                                          className="text-white bg-primary-600 rounded h-10 px-5 sm:text-sm text-xs centerOfParent">ادامه
                                        ثبت سفارش</Link>
                                    <div className="flex flex-col items-end gap-2 sm:text-sm text-xs">
                                        <span>قیمت نهایی</span>
                                        <span
                                            className="hasToman text-green-600">{isLoading ?
                                            <Spinner variant="dots" size="sm"
                                                     color="success"/> : formatCurrency(state.total_after_off)}</span>
                                    </div>
                                </div>
