...
const Filters = ({setCurrentPage}) => {
    ...
    const {slug: category, ...queries} = query
    ...
    const [data] = useGetRequest(`/product/get-filter`)
    ...
    const changeUrl = (name, value) => {
        ...
        if (str === null) {
            const {[name]: O, ...query} = query
            replace({pathname: asPath.split('?')[0], query: {...query},},
                undefined,
                {shallow: true}
            );
        } else {
            replace({pathname: asPath.split('?')[0], query: {...query, [name]: str},},
                undefined,
                {shallow: true}
            );
        }
                ...
                <div
                    ...
                    <form
                        onSubmit={(e) => {
                            ...
                            const {search, ...other} = query
                            replace({
                                pathname: asPath.split('?')[0],
                                query: value.trim().length ? {...other, search: value} : other,
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        >
                            {data.category?.map(c => <Checkbox onValueChange={isSelected => {
                                const newQuery = { ...queries };

                                if (isSelected) {
                                    newQuery.category = c.slug;
                                } else {
                    ...
                    <Dropdown
                        ...
                        Multiple Searchable label="موضوع کتاب" setState={handleFilter} name="subject"
                        placeHolder='موضوع'
                        className='!px-3 !py-2 border border-gray-400 rounded-lg bg-white'/>
                    <Dropdown
                        array={data.publication} defaultValue={filters['publication']}
                        Multiple Searchable label="انتشارات" setState={handleFilter} name="publication"
                    ...
                    <div className='px-3 py-5 flex flex-col gap-4'>
                        <div className='flex items-center justify-between'>
                            <label className="inline-flex items-center justify-between w-full cursor-pointer">
                                <input type="checkbox" name="is_used" id="is_usedField"
                                       checked={filters['is_used'] ? true : false} className="sr-only peer"
                        ...
                        <div className='flex items-center justify-between'>
                            <label className="inline-flex items-center justify-between w-full cursor-pointer">
                                <input type="checkbox" name="free_shipping" id="free_shippingField"
                                       checked={filters['free_shipping'] ? true : false} className="sr-only peer"
                        ...
                        <div className='flex items-center justify-between'>
                            <label className="inline-flex items-center justify-between w-full cursor-pointer">
                                <input type="checkbox" name="discounted" id="discountedField"
                                       checked={filters['discounted'] ? true : false} className="sr-only peer"
