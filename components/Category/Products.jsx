...
const Products = ({edu, currentPage, setCurrentPage}) => {
    ...
    const [data, setData, setReload, pagination] = useGetRequest(edu ? '/educational-product' : `/product`, currentPage, stableQuery);
                ...
                ? <>
                    {!!data.length
                        ? <div className="grid md:grid-cols-3 grid-cols-2 lg:gap-6 gap-4">
                            {data.map((d, i) => <CardC key={i} data={d} withTag={edu ? false : true}
                                                       solid={edu ? true : false} offRed={edu ? true : false} edu={edu}
                                                       withLabel={edu ? false : true}/>)}
                        </div>
