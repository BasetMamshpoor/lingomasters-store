...
const Password = ({setStep, user}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const {
            ...
            successMessage,
            errorMessage
        } = await sendPostRequest('POST', '/buyerLogin', {...data, user, grant_type: 'password'})
        if (success) {
            addToast({
                    ...
                    <form className="flex flex-col gap-20" onSubmit={handleSubmit}>
                        <div className="flex flex-col w-full gap-5">
                            ...
                            <button type='button' onClick={() => setStep(4)}
                                    className='text-right lg:text-base text-sm'>فراموشی رمز عبور؟
                            </button>
                        </div>
                        <Button isLoading={isLoading} type="submit"
                                className="text-white bg-primary-700 w-full px-3 py-2 sm:text-base text-sm rounded effect-2">
                            ورود
                        </Button>
                    </form>
                </div>
